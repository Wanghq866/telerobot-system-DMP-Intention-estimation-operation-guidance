...
def tanh(x, miu = 1, a = 130):
    x = a*x
    return miu*(np.exp(x) - np.exp(-x)) / (np.exp(x) + np.exp(-x))
...
def LoadData(tau_d):
    ...
    DMP1 = DMPLearn(data['Stouch_traj1'], ax = 0.6) # (1000*Time)*3列
    y_plan1 = DMP1.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan1 learned by DMP with Tau = %.1f",tau_d)
    
    DMP2 = DMPLearn(data['Stouch_traj2'], ax = 1.0)
    y_plan2 = DMP2.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan2 learned by DMP with Tau = %.1f",tau_d)
    
    DMP3 = DMPLearn(data['Stouch_traj3'], ax = 1.0)
    y_plan3 = DMP3.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan3 learned by DMP with Tau = %.1f",tau_d)
    
    DMP4 = DMPLearn(data['Stouch_traj4'], ax = 0.6)
    y_plan4 = DMP4.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan4 learned by DMP with Tau = %.1f",tau_d)
    
    DMP5 = DMPLearn(data['Stouch_traj5'], ax = 0.9)
    y_plan5 = DMP5.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan5 learned by DMP with Tau = %.1f",tau_d)
    
    DMP6 = DMPLearn(data['Stouch_traj6'], ax = 1.1)
    y_plan6 = DMP6.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan6 learned by DMP with Tau = %.1f",tau_d)
    
    DMP7 = DMPLearn(data['Stouch_traj7'], ax = 0.7)
    y_plan7 = DMP7.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan7 learned by DMP with Tau = %.1f",tau_d)
    
    DMP8 = DMPLearn(data['Stouch_traj8'], ax = 0.5)
    y_plan8 = DMP8.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan8 learned by DMP with Tau = %.1f",tau_d)
    
    DMP9 = DMPLearn(data['Stouch_traj9'], ax = 1.0)
    y_plan9 = DMP9.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan9 learned by DMP with Tau = %.1f",tau_d)
    
    DMP10 = DMPLearn(data['Stouch_traj10'], ax = 1.3)
    y_plan10 = DMP10.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan8 learned by DMP with Tau = %.1f",tau_d)
    
    DMP11 = DMPLearn(data['Stouch_traj11'], ax = 0.7)
    y_plan11 = DMP11.completeTraj(tau = tau_d)
    rospy.loginfo("y_plan8 learned by DMP with Tau = %.1f",tau_d)

    return y_plan1, y_plan2, y_plan3, y_plan4, y_plan5, y_plan6, y_plan7, y_plan8, y_plan9, y_plan10, y_plan11, DMP1, DMP2, DMP3, DMP4, DMP5, DMP6, DMP7, DMP8, DMP9, DMP10, DMP11
...
class DmpEvolu():
    ...
    def evolution(self,y_DMP,curpose,x,x_track,DesirTraj,pubpos,Devia,pubdevi,Tau_a_v,pubtau_a, ifpub = True):
        ...
        tau_a = self.tau_d#self.tau_d*(1+np.sign(np.inner(vector_e,vector_dmpspeed))*tanh(Exedis))
...
if __name__ == '__main__':
    ...
    y_plan1, y_plan2, y_plan3, y_plan4, y_plan5, y_plan6, y_plan7, y_plan8, y_plan9, y_plan10, y_plan11, DMP1, DMP2, DMP3, DMP4, DMP5, DMP6, DMP7, DMP8, DMP9, DMP10, DMP11 = LoadData(tau_d)
    ...
    while not rospy.is_shutdown():
        if COUNT1 < 5*100:
            ...
            rospy.loginfo("| Move to Start Point | Count:%d --> %d | Position: [%.3f,%.3f,%.3f] |",COUNT1+1, 500, 
                          PlannedTraj.position.x,PlannedTraj.position.y,PlannedTraj.position.z)
    ...
    while not rospy.is_shutdown():
        ...
        # --------- 按下按钮，开始DMP引导 ---------
            ...
            if len(ExecutedTraj) <= WINDOWS:
                ...
                y_exec4, ExecutedTraj4, cut_off4 = Cutoff(y_plan4,cutExeTraj)
                ...
                # 选中轨迹的概率
                probs = softmax(z = np.array([distance1,distance2,distance3,distance4]))
                rospy.loginfo("distance 1-2-3-4: [%.4f - %.4f - %.4f - %.4f]", distance1,distance2,distance3,distance4)
                rospy.loginfo("probs 1-2-3-4: [%.4f - %.4f - %.4f - %.4f]", probs[0], probs[1], probs[2], probs[3])
                ...
                Credit.creditability_4 = probs[3]
                Credit.creditabilityMax = max(probs)
                
                if M1 == 0 and max(probs) < LAMDA: M1 = 0
                elif (M1 == 0 and max(probs) >= LAMDA) or (M1 == 1 and max(probs) >= LAMDA and max(probs) <= 1.0):
                    M1 = 1
                    saveargmax1 = np.argmax(probs)
                #elif M1 == 1 and max(probs) >= 0.5 and max(probs) < LAMDA and saveargmax1 == np.argmax(probs): M1 = 2
                #elif M1 == 2 and max(probs) >= LAMDA and max(probs) <= 1.0: M1 = 1
                #elif M1 == 2 and max(probs) >= 0.5 and max(probs) < LAMDA and saveargmax1 != np.argmax(probs): M1 = 3
                #elif M1 == 3 and max(probs) >= LAMDA: M1 = 1
                Credit.M = M1
                pubcredit.publish(Credit)
                
                # 选择信任度最大的轨迹
                dic1 = {'y_plan1':probs[0], 'y_plan2':probs[1], 'y_plan3':probs[2], 'y_plan4':probs[3]}
                y_max_prob_traj = max(dic1,key = lambda x:dic1[x]) # 找到概率最大的轨迹-字符串
                dic2 = {'DMP1':probs[0], 'DMP2':probs[1], 'DMP3':probs[2], 'DMP4':probs[3]}
                ...
                if will_traj == willfurther:
                    if Cal_Cannio1 == True:
                        ...
                        dic3 = {'y_plan1':cut_off1, 'y_plan2':cut_off2, 'y_plan3':cut_off3, 'y_plan4':cut_off4}
                ...
                if y_max_prob_traj == savefuther:
                    if Cal_Corr1 == True:
                        ...
                        x1, tau_a, Exedis = dmpevol.evolution(y_further_DMP,curpose,x1,x_track,PlannedTraj,pubforce,Devia,pubdevia,Tau_a_v,pubtau_a)
                        
                        #rospy.loginfo("| probs 1-2-3-4: [%.3f - %.3f - %.3f - %.3f] | Select traj [%s] | Exedis = [%.3f] | tau_a = [%.3f] | Phase(x1): [%.3f] --> %.3f |",
                        #   probs[0], probs[1], probs[2], probs[3], y_max_prob_traj, Exedis, tau_a, x1, x_track[-1])
    ...
    while not rospy.is_shutdown():
        if curpose.pose[1] < y_plan1[-1,1]:
            Devia.header.stamp = rospy.Time.now()
            Devia.position.x = curpose.pose[0]-y_plan1[-1,0]
            Devia.position.y = curpose.pose[1]-y_plan1[-1,1]
            Devia.position.z = curpose.pose[2]-y_plan1[-1,2]
            pubdevia.publish(Devia)
        else:
            break
    ...
    while not rospy.is_shutdown():
        ...
        # --------- 按下按钮，开始DMP引导 ---------
            ...
            if len(ExecutedTraj2) <= WINDOWS:
                ...
                y_exec7, ExecutedTraj7, cut_off7 = Cutoff(y_plan7,cutExeTraj2)
                if cut_off5 == 0: cut_off5 = 1
                if cut_off6 == 0: cut_off6 = 1
                if cut_off7 == 0: cut_off7 = 1

                # dtw.distance_fast; cosine_similarity
                distance5 = dtw.distance_fast(y_exec5[:,2], ExecutedTraj5[:,2])
                distance6 = dtw.distance_fast(y_exec6[:,2], ExecutedTraj6[:,2])
                distance7 = dtw.distance_fast(y_exec7[:,2], ExecutedTraj7[:,2])
                
                # 选中轨迹的概率
                probs = softmax(z = np.array([distance5,distance6,distance7]))
                rospy.loginfo("distance 5-6-7: [%.4f - %.4f - %.4f]", distance5,distance6,distance7)
                rospy.loginfo("probs 5-6-7: [%.4f - %.4f - %.4f]", probs[0], probs[1], probs[2])
                ...
                Credit.creditability_2 = probs[1]
                Credit.creditability_3 = probs[2]
                Credit.creditability_4 = .0
                Credit.creditabilityMax = max(probs)
                
                if M2 == 0 and max(probs) < LAMDA: M2 = 0
                elif (M2 == 0 and max(probs) >= LAMDA) or (M2 == 1 and max(probs) >= LAMDA and max(probs) <= 1.0): 
                    M2 = 1
                    saveargmax2 = np.argmax(probs)
                # elif M2 == 1 and max(probs) >= 0.5 and max(probs) < LAMDA and saveargmax2 == np.argmax(probs): M2 = 2
                # elif M1 == 2 and max(probs) >= LAMDA and max(probs) <= 1.0: M1 = 1
                # elif M2 == 2 and max(probs) >= 0.5 and max(probs) < LAMDA and saveargmax2 != np.argmax(probs): M2 = 3
                # elif M2 == 3 and max(probs) >= LAMDA: M2 = 1
                Credit.M = M2
                pubcredit.publish(Credit)
                
                # 选择信任度最大的轨迹
                dic1 = {'y_plan5':probs[0], 'y_plan6':probs[1], 'y_plan7':probs[2]}
                y_max_prob_traj = max(dic1,key = lambda x:dic1[x]) # 找到概率最大的轨迹-字符串
                dic2 = {'DMP5':probs[0], 'DMP6':probs[1], 'DMP7':probs[2]}
                y_further_DMP = eval(sorted(dic2, key=dic2.get, reverse=True)[0]) # 找到概率最大的轨迹，获得其DMP对象
                ...
                if will_traj == willfurther:
                    if Cal_Cannio2 == True:
                        ...
                        dic3 = {'y_plan5':cut_off5, 'y_plan6':cut_off6, 'y_plan7':cut_off7}
                        for i in range(0,dic3[will_traj]):
                ...
                if y_max_prob_traj == savefuther:
                    if Cal_Corr2 == True:
                        ...
                        x2, tau_a, Exedis = dmpevol.evolution(y_further_DMP,curpose,x2,x_track,PlannedTraj,pubforce,Devia,pubdevia,Tau_a_v,pubtau_a)
                        
                        #rospy.loginfo("| probs 5-6-7: [%.3f - %.3f - %.3f] | Select traj [%s] | Exedis = [%.3f] | tau_a = [%.3f] | Phase(x2): [%.3f] --> %.3f |",
                        #   probs[0], probs[1], probs[2], y_max_prob_traj, Exedis, tau_a, x2, x_track[-1])
                        ...
                        if x2 >= x_track[-1]:
                            x2, tau_a, Exedis = dmpevol2.evolution(y_further_DMP,curpose,x2,x_track,PlannedTraj,pubforce,Devia,pubdevia,Tau_a_v,pubtau_a)
    ...
    while not rospy.is_shutdown():
        if curpose.pose[1] < y_plan5[-1,1]:
            Devia.header.stamp = rospy.Time.now()
            Devia.position.x = curpose.pose[0]-y_plan5[-1,0]
            Devia.position.y = curpose.pose[1]-y_plan5[-1,1]
            Devia.position.z = curpose.pose[2]-y_plan5[-1,2]
            pubdevia.publish(Devia)
        else:
            break
    ...
    while not rospy.is_shutdown():
        ...
        # --------- 按下按钮，开始DMP引导 ---------
            ...
            # ------------- （子空间1中一直保持记录）将已执行轨迹记录到一个数组中 ---------------
            ExecutedTraj3 = np.vstack((ExecutedTraj3, curpose.pose)) # print(ExecutedTraj) # 3列
            
            # -----------------------------------------------------------------
            if len(ExecutedTraj3) <= WINDOWS:
                continue
                
            else:
                cutExeTraj3 = ExecutedTraj3[-WINDOWS:,:]
                
                # 计算信任度
                y_exec8, ExecutedTraj8, cut_off8 = Cutoff(y_plan8,cutExeTraj3)
                y_exec9, ExecutedTraj9, cut_off9 = Cutoff(y_plan9,cutExeTraj3)
                y_exec10, ExecutedTraj10, cut_off10 = Cutoff(y_plan10,cutExeTraj3)
                y_exec11, ExecutedTraj11, cut_off11 = Cutoff(y_plan11,cutExeTraj3)
                if cut_off8 == 0: cut_off8 = 1
                if cut_off9 == 0: cut_off9 = 1
                if cut_off10 == 0: cut_off10 = 1
                if cut_off11 == 0: cut_off11 = 1

                # dtw.distance_fast; cosine_similarity
                distance8 = dtw.distance_fast(y_exec8[:,2], ExecutedTraj8[:,2])
                distance9 = dtw.distance_fast(y_exec9[:,2], ExecutedTraj9[:,2])
                distance10 = dtw.distance_fast(y_exec10[:,2], ExecutedTraj10[:,2])
                distance11 = dtw.distance_fast(y_exec11[:,2], ExecutedTraj11[:,2])
                
                # 选中轨迹的概率
                probs = softmax(z = np.array([distance8,distance9,distance10,distance11]))
                rospy.loginfo("distance 8-9-10-11: [%.4f - %.4f - %.4f - %.4f]", distance8,distance9,distance10,distance11)
                rospy.loginfo("probs 8-9-10-11: [%.4f - %.4f - %.4f - %.4f]", probs[0], probs[1], probs[2], probs[3])
                ...
                if M3 == 0 and max(probs) < LAMDA: M3 = 0
                elif (M3 == 0 and max(probs) >= LAMDA) or (M3 == 1 and max(probs) >= LAMDA and max(probs) <= 1.0): 
                    M3 = 1
                    saveargmax3 = np.argmax(probs)
                # elif M3 == 1 and max(probs) >= 0.5 and max(probs) < LAMDA and saveargmax3 == np.argmax(probs): M3 = 2
                # elif M1 == 2 and max(probs) >= LAMDA and max(probs) <= 1.0: M1 = 1
                # elif M3 == 2 and max(probs) >= 0.5 and max(probs) < LAMDA and saveargmax3 != np.argmax(probs): M3 = 3
                # elif M3 == 3 and max(probs) >= LAMDA: M3 = 1
                Credit.M = M3
                pubcredit.publish(Credit)
                
                # 选择信任度最大的轨迹
                dic1 = {'y_plan8':probs[0], 'y_plan9':probs[1], 'y_plan10':probs[2], 'y_plan11':probs[3]}
                y_max_prob_traj = max(dic1,key = lambda x:dic1[x]) # 找到概率最大的轨迹-字符串
                dic2 = {'DMP8':probs[0], 'DMP9':probs[1], 'DMP10':probs[2], 'DMP11':probs[3]}
                y_further_DMP = eval(sorted(dic2, key=dic2.get, reverse=True)[0]) # 找到概率最大的轨迹，获得其DMP对象
                ...
                if will_traj == willfurther:
                    if Cal_Cannio3 == True:
                        y_will_DMP.dmp.reset_state()
                        x_track_w3 = y_will_DMP.dmp.cs.rollout()
                        y_will_DMP.dmp.cs.reset_state()
                        
                        dic3 = {'y_plan8':cut_off8, 'y_plan9':cut_off9, 'y_plan10':cut_off10, 'y_plan11':cut_off11}
                        for i in range(0,dic3[will_traj]):
                            _, _, _, x_w3 = y_will_DMP.dmp.step(tau_d)
                        
                        dmpevol_will3 = DmpEvolu(tau_d)
                        x_w3, _, _ = dmpevol_will3.evolution(y_will_DMP,curpose,x_w3,x_track_w3,PlannedTrajWill,pubwilltraj,Devia,pubdevia,Tau_a_v,pubtau_a, ifpub = False)
                    
                        Cal_Cannio3 = False
                    else:
                        if x_w3 >= x_track_w3[-1]:
                            x_w3, _, _ = dmpevol_will3.evolution(y_will_DMP,curpose,x_w3,x_track_w3,PlannedTrajWill,pubwilltraj,Devia,pubdevia,Tau_a_v,pubtau_a, ifpub = False)
                else:
                    willflag3 = True
                    Cal_Cannio3 = True
                ...
                if y_max_prob_traj == savefuther:
                    if Cal_Corr3 == True:
                        ...
                        for i in range(0,dic3[y_max_prob_traj]):
                            # 为后边从cutoff点的DMP重放初始化（1）相位x,（2）self.y（y_further_DMP.dmp.y）和（3）self.dy（y_further_DMP.dmp.dy）
                            _, _, _, x3 = y_further_DMP.dmp.step(tau_d)
                        
                        dmpevol3 = DmpEvolu(tau_d)
                        x3, tau_a, Exedis = dmpevol3.evolution(y_further_DMP,curpose,x3,x_track,PlannedTraj,pubforce,Devia,pubdevia,Tau_a_v,pubtau_a)
                        
                        #rospy.loginfo("| probs 8-9-10-11: [%.3f - %.3f - %.3f - %.3f] | Select traj [%s] | Exedis = [%.3f] | tau_a = [%.3f] | Phase(x3): [%.3f] --> %.3f |",
                        #   probs[0], probs[1], probs[2], probs[3], y_max_prob_traj, Exedis, tau_a, x3, x_track[-1])
    ...
    while not rospy.is_shutdown():
        if COUNT3 < 5*100:
            ...
            rospy.sleep(2.)
            y_tracked = np.loadtxt('/home/sxm/project_touch_mra7a/src/omni_common/Replay_lead_trajectory_fuzzy_fusion/Two_Subspace/Touch_tracked_traj/Touch_tracked_traj.txt',delimiter=',')
